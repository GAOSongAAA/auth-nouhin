...
public class AuthProcessorImpl implements AuthProcessor {
         ...
         */
        public AuthProcessorImpl(JwtService jwtService) {
                this.jwtService = jwtService;
                logger.debug("AuthProcessorImplの初期化が完了しました");
                logger.debug("ベースURL: {}", baseUrl);
        ...
        public OauthTokenResult getOauthTokenFromEndpoint(String grant_type, String code, String redirect_uri,
                ...
                try {
                        ...
                        if (statusCode == HttpStatus.OK.value()) {
                                ...
                                // Idトークンから項目の取り出し
                                Map<String, String> items = jwtService.extractClaim(idToken, "all");
                                OauthTokenResult result = new OauthTokenResult(accessToken, true, items.get("name"),
                                                items.get("sub"), items.get("email"), items.get("given_name"),
                ...
                try {
                        ...
                        if (statusCode == HttpStatus.OK.value()) {
                                ...
                                response.addHeader("Set-Cookie",
                                                Message.Cookie.CLB_REFRESH_TOKEN + "=" + responseBody.getRefresh_token()
                                                                + ";path=/;MAX-AGE="
                                                                + COOKIE_EXPIRATION + ";SameSite=strict; ");

                                // Idトークンから項目の取り出し
                                Map<String, String> items = jwtService.extractClaim(refreshToken, "all");
                                OauthTokenResult result = new OauthTokenResult(responseBody.getAccess_token(), true,
                                                items.get("name"),
                                                items.get("sub"), items.get("email"), items.get("given_name"),
                                                items.get("family_name"));
                                return result;
