 ...
 * 設定オブジェクトの作成と初期化を行う
 */
public class LogMaskConfigFactory {

    private static final Logger logger = LoggerFactory.getLogger(LogMaskConfigFactory.class);

    // インスタンス化防止用プライベートコンストラクタ
    private LogMaskConfigFactory() {
        // インスタンス化防止
    }
     ...
     */
    public static LogMaskConfig createConfig() {
     ...
     * @return 初期化されたCommonConfigオブジェクト
     */
    public static LogMaskConfig createConfig(String configPath) {
        logger.debug("指定された設定ファイルパスを使用して設定を作成: {}", configPath);
        return createConfig(configPath, LogMaskConfig.class);
    }
