...
public class CustomJwtStrategies implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        registry.registerClaimResolver("retry", claims -> claims.get("retry-times", Integer.class));
        ...
        // 2-5  Map -> LoginRetryToken（短期）ログイン失敗時の再試行用、内部使用
        JwtTokenGenerator<Map<String, Object>> loginRetryGenerator = map -> Jwts.builder()
                .addClaims(map)
                .setIssuer(ConfigManager.getConfig().getCollaboportalIssuer())
                .setIssuedAt(new Date())
                .setExpiration(new Date(System.currentTimeMillis()
                ...
                .signWith(secretKey())
                .compact();
        registry.registerTokenGenerator(JwtConstants.RETRY_TYPE_LOGIN, loginRetryGenerator);
        ...
        JwtTokenValidator strictValidator = (token, claims) ->
                claims.getExpiration().after(new Date())
                        && ConfigManager.getConfig().getCollaboportalIssuer().equals(claims.getIssuer());
        registry.registerTokenValidator(JwtConstants.VALIDATE_TYPE_ISSUER, strictValidator);
